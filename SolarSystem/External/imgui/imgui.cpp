...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar))
		{
			...
			text_min.x += pad_left+10.0f;//Indent title's text little bit more
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (hovered || selected)
	{
		const ImU32 col = GetColorU32((held || selected) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
...
namespace ImGui//Added functions
{
	...
	bool StateSmallButtonWithTooltip(const char* buttonText, const char* tooltipText, bool pressed)
	{
		...
		ImGui::PushStyleColor(ImGuiCol_Button,
							  pressed ? style->Colors[ImGuiCol_ButtonHovered] : style->Colors[ImGuiCol_Button]);
		bool state = ImGui::SmallButton(buttonText);
		ImGui::TextTooltipOnHover(tooltipText);
		ImGui::PopStyleColor();
		return state;
	};
